...
def filter_physical_datasets(datasets: list) -> list:
    """Filter out SQL-based datasets, keeping only physical table-based datasets.

    SQL datasets (created from SQL queries) have sql attribute set and no data_source_table_id.
    These are not generated by generate_logical_model which only works with physical tables.
    """
    return [ds for ds in datasets if ds.data_source_table_id is not None]
...
def test_generate_logical_model(test_config: dict):
    ...
    Also, workspace LDM may contain SQL-based datasets that are not generated from physical tables.
    """
    # Filter out SQL-based datasets as they're not generated by generate_logical_model
    workspace_datasets = filter_physical_datasets(declarative_model.ldm.datasets)
    assert workspace_datasets == generated_declarative_model.ldm.datasets
...
def test_scan_pdm_and_generate_logical_model(test_config: dict):
    ...
    # Verify the scan produced valid LDM with expected tables
    # Note: Exact comparison with workspace LDM is not possible because scan_pdm_and_generate_logical_model
    # produces a "raw" LDM without FK references, while workspace LDM may have enhanced references.
    assert generated_declarative_model is not None
    assert (
        len(generated_declarative_model.ldm.datasets) >= 5
    )  # campaign_channels, campaigns, customers, order_lines, products
    assert len(generated_declarative_model.ldm.date_instances) >= 1  # At least one date instance

    # Verify expected dataset IDs are present
    dataset_ids = {ds.id for ds in generated_declarative_model.ldm.datasets}
    expected_ids = {"campaign_channels", "campaigns", "customers", "order_lines", "products"}
    assert expected_ids.issubset(dataset_ids), f"Missing datasets: {expected_ids - dataset_ids}"
...
def test_scan_pdm_and_generate_logical_model_with_sql_datasets(test_config: dict):
    ...
    # Verify the generation produced valid LDM including SQL datasets
    assert generated_declarative_model is not None
    dataset_ids = {ds.id for ds in generated_declarative_model.ldm.datasets}

    # Verify physical tables are present
    expected_physical_ids = {"campaign_channels", "campaigns", "customers", "order_lines", "products"}
    assert expected_physical_ids.issubset(dataset_ids), (
        f"Missing physical datasets: {expected_physical_ids - dataset_ids}"
    )

    # Verify SQL datasets are present (from build_pdm_sql_datasets)
    # These are datasets generated from the SQL statements provided in the request
    assert len(generated_declarative_model.ldm.datasets) > len(expected_physical_ids), (
        "SQL datasets should be generated"
    )
    assert len(generated_declarative_model.ldm.date_instances) >= 1
...
def test_catalog_list_data_sources(test_config):
    ...
    # Check expected data source exists (environment may have additional data sources)
    assert len(data_sources) >= 1
    expected_ds = _get_data_source(data_sources, test_config["data_source"])
    assert expected_ds is not None
    assert expected_ds.id == test_config["data_source"]
...
def test_catalog_create_update_list_data_source(test_config):
    sdk = GoodDataSdk.create(host_=test_config["host"], token_=test_config["token"])
    try:
        # Get initial state (environment may have additional data sources)
        initial_data_sources = sdk.catalog_data_source.list_data_sources()
        initial_count = len(initial_data_sources)
        assert initial_count >= 1
        assert _get_data_source(initial_data_sources, test_config["data_source"]) is not None
        ...
        data_sources = sdk.catalog_data_source.list_data_sources()
        assert len(data_sources) == initial_count + 1  # One new DS added
        ...
        data_sources = sdk.catalog_data_source.list_data_sources()
        assert len(data_sources) == initial_count  # Back to initial state
...
def test_catalog_patch_data_source(test_config):
    ...
    try:
        ...
        data_sources = sdk.catalog_data_source.list_data_sources()
        assert len(data_sources) >= 1
        assert _get_data_source(data_sources, test_config["data_source"]) is not None
